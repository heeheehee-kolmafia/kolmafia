...
public class ClanBuffRequest extends GenericRequest {
  private final int buffId;

  /**
   * Constructs a new <code>ClanBuffRequest</code> with the specified buff identifier. This
   * constructor is only available internally. Note that no descendents are possible because of the
   ...
   *
   * @return A complete <code>List</code>
   */
  public static final List<ClanBuffRequest> getRequestList() {
    List<ClanBuffRequest> requestList = new ArrayList<ClanBuffRequest>();
    for (int i = 1; i < 9; ++i) {
      for (int j = 1; j <= 3; ++j) {
  ...
  /**
   * Returns the string form of this request, which is the formal name of the buff that this buff
   * request represents.
   *
